...
$(document).ready(function () {
    ...
    var chat_object_click = function (chat_object_name) {
        // 修改聊天对象的名字
        $("#chat_object_now").text(chat_object_name);
        ...
        // 消除未读消息数
        $(updata_dom_id + " .new-messages-count").remove();
        ...
        // 从本地存储中获取以前保存的消息
        if (localStorage.getItem("messages" + chat_object_name) == undefined) {
        ...
        // 获取并加载新消息
        var new_messages = get_chat_object_new_contents_by_name(chat_object_name);
        if (new_messages == undefined || new_messages.length == 0) {
            console.log("获取到的新消息为空");
            return;
        }
        ...
        clear_chat_object_new_contents_by_name(chat_object_name); // 清空新消息,否则会重复加载， 我就是将获取的消息都放到本地，然后data里面置空
    ...
    var chat_send_message = function (chat_object_name) {
        ...
        // TODO:发送消息到服务器
        // const sendMessageSettings = {
        //     "async": false,
        //     "crossDomain": true,
        //     "url": server_ip_port + "chat/sendMessage",
        //     "method": "POST",
        //     "headers": {
        ...
        // $.ajax(sendMessageSettings).done(function (response) {
        //     console.log(response);
        // });
        const sendMessageSettings = {
            "async": false,
        ...
        console.log(sendMessageSettings.data)
        console.log(typeof sendMessageSettings.data)
    ...
    var update_chat_object_list = function (object_list) {
        ...
        $.each(object_list, function (index, object) {
    ...
    var data_get_chat_object_list = function (build_dom, click) {
        $.ajax({
            type: "POST",
            async: false,
            ...
            success: function (response) {
                ...
                // // 绑定事件
                // if (bind_event == undefined) {
                //     console.log("bind_event is undefined");
                //     return
                // }
                // bind_event();
                ...
                // if (click == undefined) {
                //     console.log("click is undefined");
                //     return
                // }
                // // 点击第一个聊天对象, 在data.chat_object_list消除未读消息数和未读消息，并且更新聊天记录
                // click(data.chat_object_list[0].user_name);
